...
async def main():
    init_config()
    init_session()
    text = f'启动成功，直播间：{ROOM_IDS}，语音模式：{MODE}，心跳间隔：{HEART_PRINT}，连续礼物间隔：{GIFT_INTERVAL}秒'
    print(text)
    # speech(text)
    try:
...
def init_config():
    ...
    BLACK_USER = config['black_user']
    BLACK_TEXT = config['black_text']
    if MODE == 'azure':
        init_azure_config()
    if MODE == 'alibaba':
        init_alibaba_config()
...
class MyHandler(blivedm.BaseHandler):  # 类变量，将被所有类的实例共享
    ...
    def _on_enter(self, client: blivedm.BLiveClient, data: web_models.UserInData):
        if data.medal_level >= WELCOME_LEVEL:
            ...
            if (len(VOICE_TEXT["enter"]) > 0):
                if (data.uname not in BLACK_USER):
            ...
            if (len(VOICE_TEXT["danmaku"]) > 0):
                ...
                for word in BLACK_TEXT:
                    if (msg.find(word) >= 0):
                        ...
                        break
                if(isBlackText):
                    print(f'   屏蔽词：{blackText}')
                elif (message.uname in BLACK_USER):
                    print(f'   黑名单用户发言：{message.uname}')
                else:
    ...
    def _on_red_pocket(self, client: blivedm.BLiveClient, message: web_models.GiftMessage):
        ...
        print(f' {message.uname} 赠送 {message.gift_name}x{message.num}'
              f' （价值x{message.price}）')
        if (len(VOICE_TEXT["gift"]) > 0 and message.uname not in BLACK_USER):
    ...
    def _on_buy_guard(self, client: blivedm.BLiveClient, message: web_models.GuardBuyMessage):
        ...
        speech(f' 感谢 {message.username} 购买 {message.gift_name}')

    # 点赞消息处理：PS：可能存在并发问题
    def _click_like(self, client: blivedm.BLiveClient, data: web_models.ClickData):
        if len(data.uname) != 0:
            print(f' {data.uname} {data.like_text}')
            if (len(VOICE_TEXT["like"]) > 0 and data.uname not in BLACK_USER):
            ...
            while True:
                if index > 0 and LIKE_NUMS[index] > data.click_count > LIKE_NUMS[index - 1]:
                    ...
                    if (len(VOICE_TEXT["like_total"]) > 0):
    ...
    def _on_super_chat(self, client: blivedm.BLiveClient, message: web_models.SuperChatMessage):
        ...
        speech(f' 醒目留言 {message.uname}说：{message.message}')
